...
flights$time_in_air <- flights$arr_delay - flights$dep_delay

# Sort your data.frame desceding by the column you just created
flights <- arrange(flights, desc(time_in_air))
View(flights)

# Try doing the last 2 steps in a single operation using the pipe operator
flights <- mutate(flights, time_in_air = arr_delay - dep_delay) %>% arrange(desc(time_in_air))
View(flights)

# Make a histogram of the amount of gain using the `hist` command
hist(flights$time_in_air)
